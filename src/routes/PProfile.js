    ...
    arr.map((item) => (
      ...
      >
        <div>
          ...
          <span className="id">
            {(name === '게시물' && `에 #${item.id} 게시글을 작성하였습니다.`) ||
              (name === '댓글' && `#${item.id} 게시글에 댓글을 남겼습니다.`) ||
              (name === '스크랩' && `#${item.id} 게시글을 스크랩 하였습니다.`)}
          </span>
          <span className="date">
...
const Profile = () => {
  ...
  console.log('Profile locataion', memberId);
  ...
  const { loading, userInfo, posts, comments, scraps } = state;
  ...
  const { editT, postT, cmtT, scrapT } = toggle;
  ...
  const fetchData = async (num) => {
    ...
    try {
      ...
      const { data: s } = await memberApi.getUserScraps(id, num);
      console.log('Profile scraps', s);
      setState({
        ...state,
        userInfo,
        loading: false,
  ...
  const fetchContents = async (num) => {
    ...
    try {
      ...
      if (scrapT) {
        const { data: s } = await memberApi.getUserScraps(id, num);
        console.log('fetchContents scraps', s);
        setState({
          ...state,
          userInfo,
          loading: false,
  ...
  const onToggle = (txt) => {
    ...
    if (!toggle.postT && txt === 'posts') {
      setToggle({ ...toggle, postT: true, cmtT: false, scrapT: false });
      fetchContents(1);
      setPage([1, 2, 3, 4, 5]);
    }
    if (!toggle.cmtT && txt === 'cmts') {
      setToggle({ ...toggle, postT: false, cmtT: true, scrapT: false });
      fetchContents(1);
      setPage([1, 2, 3, 4, 5]);
    }
    if (!toggle.scrapT && txt === 'scraps') {
      setToggle({ ...toggle, postT: false, cmtT: false, scrapT: true });
      fetchContents(1);
      setPage([1, 2, 3, 4, 5]);
    }
  };
      ...
      {!loading ? (
        <UserActivity>
          <Activity>
            ...
            {scrapT && (
              <>
                <Comp arr={scraps.contents} name="스크랩" />
                {scraps.totalElements > 0 && (
                  <PageList
                    page={page}
                    setPage={setPage}
                    fetchContents={fetchContents}
                    totalPage={scraps.totalPage}
                    currentPage={scraps.currentPage}
                  />
                )}
              </>
            )}
          ...
          <ToggleList>
            ...
            <Item active={scrapT} onClick={() => onToggle('scraps')}>
              스크랩
            </Item>
          </ToggleList>
        </UserActivity>
