...
const NoticeBoard = ({ id }) => {
  ...
  let { data: noticeCounts, refetch } =
    useQuery('NoticeCounts', () => memberApi.getNoticeCounts(id).then(res => res.data));
  ...
  const {
    isLoading,
    data,
  } = useQuery(['notices', page], () => fetchProjects(page), { keepPreviousData: true });
  ...
  const readOneNotice = (noticeId, boardType, boardId) => {
    ...
    const type = (boardType === 'QNA' && 'qna') ||
      (boardType === 'TECH' && 'tech') ||
      (boardType === 'FREE' && 'free');
    memberApi.readNotices(id, noticeId, t, boardId)
      .then(res => {
    ...
    <Container>
      <NoticeHeader>
        <div>
          읽지 않은 알림 <span style={{
            color: '#fa5252',
            fontWeight: 'bold'
          }}>{noticeCounts?.count}</span> 개
        </div>
        <div
          onClick={mutate}
          style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}
        >
          <div className='check'><FaCheck /></div>모두 읽음 표시
        </div>
      </NoticeHeader>
      ...
      {data?.content?.map(n => (
        <Post
          key={n.noticeId}
          checked={n.checked}
          fci={n.noticeId === data?.content[data?.content.length - 1].noticeId}
        >
          <div>
            <Top>
              <div className='icon'>
              ...
              <span>
                {
                  (n.messageType === 'COMMENT' && '게시물에 댓글을 등록했습니다.') ||
                  (n.messageType === 'COMMENT_LIKE' && '게시물 회원님의 댓글에 좋아요를 눌렀습니다.') ||
                  (n.messageType === 'BOARD_LIKE' && '게시물에 좋아요를 눌렀습니다.') ||
                  (n.messageType === 'SELECTION' && '게시물 회원님의 댓글을 채택했습니다.')
                }
              </span>
              <span className='date'>{`${n.createdDate.split('T')[0]
                } ${n.createdDate.split('T')[1].substring(0, 8)}`}</span>
            </Top>
            <Bottom onClick={() => readOneNotice(n.noticeId, n.boardType, n.boardId)} >
              {n.title}
            </Bottom>
          </div>
        </Post> 
